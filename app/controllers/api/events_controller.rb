class Api::EventsController < ApplicationController
  ...
  def index
    ...
    if page < 1 or limit < 1
      ...
      return render json: PublicFunction.data_json('success', 'Event list', @events.size, @events)
    end
  ...
  def show
    ...
    if Event.where(:id => event_id).size > 0
      render json: PublicFunction.data_json('success', 'Show events', 1, Event.find(event_id))
    else
  ...
  def event_photo
    event_id = params[:event_id]
    if Event.where(:id => event_id).size > 0
      render json: PublicFunction.data_json('success', 'List events images', Event.find(event_id).event_images.size, Event.find(event_id).event_images)
  ...
  def join_event
    if !params[:user_id].nil? and !params[:event_id].nil?
      if Event.where(:id => params[:event_id]).size < 1
        return render json: PublicFunction.data_json('failed', 'Event not found', 0, nil)
      end
      ...
      return render json: PublicFunction.data_json('success', 'Joined', 1, @join_event)
